...
import { History, Download, Trash2, Settings2 } from 'lucide-react';
...
export const HistoryWidget = () => {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {showControls && (
        <div className="flex flex-col gap-2 p-2 rounded-xl border border-[var(--widget-accent)]/30 bg-[var(--widget-accent)]/5 animate-fadeIn mb-2">
      ...
      <div className="space-y-3 mt-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
        {Object.entries(groupedHistory).length === 0 ? (
          <p className="py-4 text-center text-xs text-[var(--page-muted)]">No matching history entries.</p>
        ) : (
          Object.entries(groupedHistory).map(([domain, items]) => (
            <div key={domain} className="rounded-xl border border-[var(--surface-line)] bg-[var(--surface-strong)]/30 p-3 hover:border-[var(--widget-accent)]/30 transition-colors">
              <div className="mb-2 text-[10px] uppercase tracking-[0.2em] font-bold text-[var(--widget-accent)]">{domain}</div>
              <ul className="space-y-1">
                {items.map((item) => (
                  <li key={item.id} className="text-xs text-[var(--page-ink)] truncate hover:text-[var(--widget-accent)] transition-colors cursor-pointer">
